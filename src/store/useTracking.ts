...
export const useTracking = create<TrackingState>()(
  persist(
    (set, get) => ({
       ...
       * วันที่ออกติดมา · บันทึกเก่าว่าเคยออกจากลานนี้ พอข้อมูลผิด รถที่จอดอยู่จริง
       * ก็หายจากงานทุกสถานี และไปโผล่ในการ์ด Gate-out แทน
       *
       ...
       * เพราะเป็นแผน ไม่ใช่บันทึกว่าออกไปแล้ว (และไม่ทำให้อ่านเป็น Gate-out อยู่
       * แล้วด้วย) แล้วประทับคำยืนยันของคนที่ยืนอยู่ลานนี้ว่ารถอยู่ — คำยืนยันนี้
       * ชนะบันทึกเก่าว่าเคยออก (ดู departureFromSite) รถจึงหลุดจากการ์ด Gate-out ทันที
       *
